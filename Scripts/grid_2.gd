...
func _ready():
	...
	for row in range(GRID_HEIGHT):
		grid[row][0].setState(Cell.State.BLOCKED, null)
		grid[row][GRID_WIDTH-1].setState(Cell.State.BLOCKED, null)
	
	for col in range(GRID_WIDTH):
		grid[GRID_HEIGHT-1][col].setState(Cell.State.BLOCKED, null)
	
	@warning_ignore("integer_division")
	currentHexomino = hexomino.new(Vector2(3,GRID_WIDTH/2), Direction.TOP, HexType.O, texture_O);
	drawHexomino();
...
func undrawHexomino():
	...
	for i in range(4):
		var localCell = localPositions[i]
		grid[localCell.x][localCell.y].setState(Cell.State.FREE, null)
...
# Doesn't work with different textures...
# The grid should be a double array of hexominos instead.
func removeLine(row: int):
	if (row == 0):
		for col in range(1, GRID_WIDTH-1):
			grid[row][col].setState(Cell.State.FREE, null);
	else:
		for col in range(1, GRID_WIDTH-1):
			grid[row][col].setState(grid[row-1][col].state, grid[row-1][col].texture);
		removeLine(row-1)
...
func removeInverseLine(row: int):
	if (row == 0):
		...
		for col in range(1, GRID_WIDTH-1):
			grid[row][col].setState(grid[row-1][col].state, null);
		removeLine(row-1)
